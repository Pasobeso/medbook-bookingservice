...
RUN cargo build --frozen --release --bin medbook-bookingservice
...
COPY --from=builder /app/target/release/medbook-bookingservice /usr/local/bin/medbook-bookingservice
...
EXPOSE 3000
ENTRYPOINT ["/usr/local/bin/medbook-bookingservice"]
