...
use crate::{
    domain::{
        entities::appointments::AddAppointmentEntity,
        repositories::appointment_ops::AppointmentOpsRepository,
    },
    infrastructure::postgres::{
        postgres_connection::PgPoolSquad,
        repositories::data_access_objects::{
            appointment_ops::AppointmentOpsDao, appointment_viewing::AppointmentViewingDao, slot_ops::SlotOpsDao, slot_viewing::SlotViewingDao
        },
    },
};
...
impl AppointmentOpsRepository for AppointmentOpsPostgres {
        ...
        let appointment_id = conn
            .transaction(|conn| {
                async move {
                    ...
                    let slot_is_not_full = SlotOpsDao::try_add_slot_appointment_count(conn, slot_id).await?;
    ...
    async fn remove(&self, appointment_id: Uuid, patient_id: i32) -> Result<()> {
        ...
        conn.transaction(|conn| {
            async move {
                let slot_id = AppointmentViewingDao::get_slot_id_by_appointment_id(conn, appointment_id).await?;
                SlotOpsDao::lock(conn, slot_id).await?;
